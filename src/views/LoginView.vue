         ...
         <div class="bg-slate-100 text-black">
          <div class="w-full h-[100vh] flex items-center overflow-x-hidden relative">
            ...
            <div class="w-full h-full child:font-Roboto">
                 <div class="w-full h-full flex items-center justify-center flex-col font-Roboto">
                    <h1 class="font-extrabold text-[3rem] tracking-tight">Sign in to BitCloud</h1>
                    <p class="mt-5 text-[.8rem] text-slate-500">Lorem ipsum dolor sit amet consectetur adipisicing elit.</p>
                    <div class="text-green-500 flex items-center p-3 px-10 mt-2 bg-slate-200 rounded-full"> 
                      ...
                      https://<span class="text-black">accounts.bitcloud.com/login</span></div>
                      <div class="flex items-center gap-x-5 cursor-pointer my-7">
                        <span @click="cheangTypeInput($event)" id="emailbtn" class="py-2 px-6 inline-flex rounded-full text-sm text-white bg-black">Email</span>
                      ...
                      <form class="flex flex-col w-[25rem]">
                       ...
                       <div  class="mb-5" v-if="TypeSendData">
    <label for="mobile" class="text-sm text-slate-300">MOBILE</label>
    <div class="flex items-center gap-x-2">
        <button id="dropdown-phone-button" data-dropdown-toggle="dropdown-phone" class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-900 bg-gray-100 border-2 border-gray-300 rounded-lg hover:bg-gray-200  outline-none" type="button">
        ...
        <div id="dropdown-phone" class="z-10 hidden bg-transparent divide-y divide-gray-100 rounded-lg shadow w-52 dark:bg-gray-700">
        ...
        <label for="phone-input" class="mb-2 text-sm font-medium text-slate-300 sr-only ">Phone number:</label>
        <div class="relative w-full">
            <input type="phone" id="phone-input" @keyup="inputvaled($event)" class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-transparent rounded-lg   border-slate-300 border-2 outline-none " pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" placeholder="123-456-7890" required />
        </div>
    </div>
                      ...
                      <button @click="activelogin()" class="text-white bg-blue-500 w-[25rem] rounded-full py-2">Login</button>
                 </div>
            </div>
            ...
            <div class="absolute w-full h-[7rem] top-0 left-0 " data-aos="flip-up" data-aos-duration="1000">
              <div class="w-full h-full flex  items-center  tablet:flex-row-reverse flex-col-reverse tablet:justify-between gap-y-4 tablet:gap-y-0 pt-5 tablet:pt-0  tablet:px-12">
                ...
                <div class="flex items-center flex-row-reverse gap-x-1">
                  <h2 class=" desktop:text-white font-Roboto font-extrabold text-2xl">BitCloud</h2>
                  <img src="../assets/image/logo symbol.png" class="w-10 h-10 object-cover" alt="logo">
                </div>
              </div>
            </div>
          </div>
...
<script>
import MakerIsToast from '../js/utils'
   export default {
    components : {
    },
    data () {
    ...
    methods : {
      ...
      inputvaled : function (event) {
            const valueinputnumber = event.target.value.trim();
            const regex_valed = /^0\d{10}$/g
            const resultType =  regex_valed.test(valueinputnumber);
            if (resultType) {
                this.mobilevaled = true
            }
          },
          activelogin : function () {
        if (this.TypeSendData) {
          if (this.mobilevaled && this.$refs.inputpassword.value) {
            this.$router.replace('/');
          }else {
            MakerIsToast('Warning' , 'Please enter valid password AND mobile number')
          }
        }
      },
